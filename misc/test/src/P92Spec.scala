...
class P92Spec extends AnyFunSpec:
  ...
  it("von koch's conjecture"):
    ...
    forAll(data) { edges =>
      val labels   = P92.vonKochLabeling(edges)
      val vertices = edges.flatMap(e => Set(e.u, e.v))
      labels.keySet shouldBe vertices
      labels.values.toSet should have size vertices.size
      all(labels.values) should (be >= 1 and be <= vertices.size)
    }
