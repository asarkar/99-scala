...
are in the closed range [1,n], and the edge labels are in the closed range [1,n-1]. See the
[image](misc/test/resources/92.jpg) for examples.

This problem can be solved by DFS with backtracking. We try vertices with higher degrees first
